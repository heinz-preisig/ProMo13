...
class HomeWindow(QtWidgets.QMainWindow):
  def __init__(self):
    ...
    self.setDockOptions(self.GroupedDragging |
                        self.AllowTabbedDocks | self.AllowNestedDocks)
    ...
    self.r_bottom_dock = QtWidgets.QDockWidget(
        "Python console", self.right_frame)
    self.r_bottom_dock.setAllowedAreas(
        Qt.RightDockWidgetArea | Qt.BottomDockWidgetArea | Qt.RightDockWidgetArea)
    ...
    # ========= left bottom frame ============
    self.label_left_bottom_frame_heading = QtWidgets.QLabel(
        self.l_bottom_frame)
    ...
    toggle_opened_tools_action.triggered.connect(
        partial(self.toggle_frames, "opened_tools"))
    toggle_python_shell_action = QtWidgets.QAction("Python Shell", self)
    toggle_python_shell_action.triggered.connect(
        partial(self.toggle_frames, "python_shell"))
    toggle_error_console_action = QtWidgets.QAction("Error Console", self)
    toggle_error_console_action.triggered.connect(
        partial(self.toggle_frames, "error_console"))

    view_menu.addActions((toggle_opened_tools_action,
                         toggle_python_shell_action, toggle_error_console_action))
  ...
  def ask_help(self, program_name):
    ...
    """
    help_pdf_path = os.path.join(
        help_dir, f"{MAP_HELP_FILE[program_name]}.pdf")
