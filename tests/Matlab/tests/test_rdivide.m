...
%! S1 = 20;
%! S2 = MultiDimVar({}, [1], [10]);
%! expected_output = MultiDimVar({}, [1], [2]);
% ACTION
%! test_output = S1 ./ S2;
% ASSERT
...
% SETUP
%! S = 20;
%! V = MultiDimVar({"A"}, [3], [10; 5; 4]);
%! expected_output = MultiDimVar({"A"}, [3], [2; 4; 5]);
% ACTION
%! test_output = S ./ V;
% ASSERT
%! assert(isequal(test_output,expected_output))
...
% SETUP
%! S = MultiDimVar({}, [1], [2]);
%! M = MultiDimVar({"A", "B"}, [3, 2], [10 4 6; 0 8 2]);
%! expected_output = MultiDimVar({"A", "B"}, [3, 2], [5 2 3; 0 4 1]);
% ACTION
%! test_output = M ./ S;
% ASSERT
%! assert(isequal(test_output,expected_output))
...
% SETUP
%! V1 = MultiDimVar({"A"}, [3], [10; 4; 8]);
%! V2 = MultiDimVar({"A"}, [3], [5; 2; 4]);
%! expected_output = MultiDimVar({"A"}, [3], [2; 2; 2]);
% ACTION
%! test_output = V1 ./ V2;
% ASSERT
%! assert(isequal(test_output,expected_output))
...
% SETUP
%! M1 = MultiDimVar({"A", "B"}, [3, 2], [10 4 6; 0 8 2]);
%! M2 = MultiDimVar({"A", "B"}, [3, 2], [2 2 2; 2 2 2]);
%! expected_output = MultiDimVar({"A", "B"}, [3, 2], [5 2 3; 0 4 1]);
% ACTION
%! test_output = M1 ./ M2;
% ASSERT
