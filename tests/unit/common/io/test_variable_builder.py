...
class CasesVariableBuilding:
    def valid_data_ok(
        ...
        fake_data_io.set_variable_data(GOOD_DATA)

        indices = {}
        for idx_data in INDEX_DATA:
        ...
        init_indices = {}
        for var_data in GOOD_DATA:
            identifier = var_data["identifier"]
            if "indices" in var_data:
                init_indices["indices"] = [indices[idx] for idx in var_data["indices"]]

            new_var = Variable(**(var_data | init_indices))
            expected_variable_map[identifier] = new_var

        return fake_io_manager, expected_variable_map
