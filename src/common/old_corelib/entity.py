...
from src.common.old_corelib import Equation, Variable
...
class EntityMathGraph:
    ...
    def equations(self) -> list[Equation]:
        ...
        return [node for node in self._graph.nodes() if isinstance(node, Equation)]

    @property
    def variables(self) -> list[Variable]:
        """All the variables in the graph"""
        return [node for node in self._graph.nodes() if isinstance(node, Variable)]
    ...
    def removable_variables(self) -> list[Variable]:
        ...
        """
        return [
            var for var in self._output_variables if self._graph.in_degree(var) == 0
        ]
    ...
    def add_equation(self, dependent_var: Variable, eq: Equation) -> list[Variable]:
    ...
    def _remove_node(self, node_to_remove: Variable | Equation) -> list[Variable]:
        """Remove a node
        ...
        return [node for node in removed_nodes if isinstance(node, Variable)]

    def _is_node_disconnected(self, node: Variable | Equation) -> bool:
    ...
    def get_equation_for_variable(self, var: Variable) -> Equation | None:
...
class Entity:
    ...
    def update_variable(
        ...
        new_equation: Equation | None = None,
    ) -> None:
        """Update a variable
    ...
    def _update_variable_equation(
        self,
        var: Variable,
        new_equation: Equation | None,
        old_equation: Equation | None,
    ) -> None:
        ...
        if new_equation is not None:
            variables_from_equation = self._math_graph.add_equation(var, new_equation)
            variables_to_add = [
                var
        ...
        if old_equation is not None:
            variables_to_delete = self._math_graph.remove_equation(old_equation)
