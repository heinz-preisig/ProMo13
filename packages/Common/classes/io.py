...
from Common.classes import equation_parser
...
def load_translation_info_from_file(language: str) -> TranslationInfo:
...
def save_arc_options_to_file(
        ontology_name: str, arc_options: dict[str, dict[str, str]]
        ):
  # TODO: Merge it with write entities to file
  path = resource_initialisation.FILES["arc_options"] % ontology_name
  with open(
          path,
          "w",
          encoding="utf-8",
...
def translate_equations(ontology_name: str, language: str):
  ...
  all_variables, all_indices, all_equations = load_var_idx_eq_from_file(ontology_name)
...
def load_entities_from_file(
  ...
  # """
  path = (
          resource_initialisation.FILES["variable_assignment_to_entity_object"]
          % ontology_name
  )
...
def load_topology_objects(
        ...
        ) -> dict[str, old_topology.TopologyObject]:
  path_old = resource_initialisation.FILES["model_file"] % (
          ontology_name,
          model_name,
          )
  path_new = path_old[:-5] + "_new.json"
  custom_decoder = old_topology.custom_decoder_factory(all_entities)
  with open(
          path_new,
          encoding="utf-8",
          ) as file:
...
def save_topology_objects(
  ...
  with open(
    ...
    json.dump(
            all_topology_objects, file, cls=modeller_classes.CustomJSONEncoder, indent=4
            )
...
def get_available_models(ontology_name: str) -> list[str]:
  ontology_path = (
          resource_initialisation.DIRECTORIES["ontology_location"] % ontology_name
  )
...
def convert_model_file(ontology_name, model_name):
  ...
  if "instantiation_info" not in data:  # 2023-07-10 HAP
    data["instantiation_info"] = {"nodes": {}, "arcs": {}}
  ...
  for arcID in data["arcs"]:
    data["instantiation_info"]["arcs"][arcID] = data["arcs"][arcID][
      "instantiated_variables"
    ]

  if "instantiation_info" not in new_data:  # 2023-07-10 HAP
    new_data["instantiation_info"] = {"nodes": {}, "arcs": {}}
  ...
  for node_id in data["instantiation_info"]["nodes"]:
    ...
    if node_id != "1":
      ...
      nr += 1
    new_data["instantiation_info"]["nodes"][new_node_id] = data[
      "instantiation_info"
    ]["nodes"][node_id]
...
def generate_latex_images(ontology_name):
  ...
  for eq_id, eq in all_equations.items():
    ...
    latex_info[eq_id] = (
            "$"
            + latex_translation.get("lhs")
            + "="
            + latex_translation.get("rhs")
            + "$"
    )
  ...
  for file_name, latex_alias in latex_info.items():
    ...
    subprocess.run(
            ["latex", "-interaction=nonstopmode", file_name + ".tex"],
