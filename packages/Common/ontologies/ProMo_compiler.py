...
__since__ = "2022. 04. 17 Extracted for compiling from knowledge graph -- no checks done"
...
if __name__ == '__main__':
  ...
  ProMoOnto = ProMoOntology("var_equ_rdf.ttl")
  res = ProMoOnto.getAllVariablesAndExpressions()
  equation_dictionary = {}
  for iri, label, expr_list, network in res:
    # print(iri,label,expr_list)
    s = ProMoOnto.extractProMoExpression(expr_list)
    e_ID = expr_list.split("_")[-1]
    equation_dictionary[e_ID] = (label, s, network)
    # print("equation %s"%e_ID, "variable: %s "%label, ":=", s)

  for e_ID in equation_dictionary:
    label, expr, network = equation_dictionary[e_ID]
    print("equation %s  " % e_ID, network, "-", label, ":=", expr)