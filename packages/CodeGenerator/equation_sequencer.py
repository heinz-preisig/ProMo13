...
def sequence_equations(
    ...
    """
    topology_graph = old_topology.build_topology_graph(all_topology_objects)

    equation_graph, equation_topology_map = _build_equation_graph(topology_graph)
    ...
    while processing_queue:
        ...
        for topology_obj in topology_objects:
            equations_info = _find_dependencies(
                source_equation_id, topology_obj, topology_graph
            )
            for equation_id, topology_objects in equations_info.items():
                ...
                new_topology_objects = (
                    topology_objects - equation_topology_map[equation_id]
                )
...
def _find_dependencies(
    equation_id: str,
    topology_obj: old_topology.EntityContainer,
    topology_graph: nx.Graph,
    ...
    * The variables have an equation: The equation and the location of the
    ...
    if entity_instance.is_input(variable_id):
        ...
        for neighbor in topology_graph[topology_obj]:
            neighbor_entity: old_corelib.Entity = neighbor.entity_instance
...
def _find_solving_order(equations_graph: nx.DiGraph) -> list[set[str]]:
    ...
    correct_order = [
        condensed_graph.nodes[node]["members"] for node in reversed(topological_order)
    ]
