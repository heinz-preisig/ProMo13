...
class EquationParser:
  ...
  def p_expr_variable(self, p: yacc.YaccProduction) -> None:
    ...
    if var_id not in self.variables:
      raise ValueError(
          f"Variable {var_id} not found in equation {self.eq_id}"
      )
    var_name = self.variables[var_id].get_label(self.language)
    # TODO: Load the language specific indexes
    var_indices = []
  ...
  def p_expr_reduce_product(self, p: yacc.YaccProduction) -> None:
    ...
    idx1 = self.indices[p[3]].get_alias(self.language)
  ...
  def p_expr_block_reduce_product(self, p: yacc.YaccProduction) -> None:
    ...
    idx1 = self.indices[p[3]].get_alias(self.language)
    idx2 = self.indices[p[5]].get_alias(self.language)
  ...
  def get_translation(self, item):
    ...
    if translation is None:
      raise ValueError(
          f"Translation not found for item: {item} in equation {self.eq_id}"
      )
