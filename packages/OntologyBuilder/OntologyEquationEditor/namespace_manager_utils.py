"""
A set of utilities (+spare) used by the namespace manager module  
...
import sqlite3, urllib.parse, webbrowser, os
...
myDB_Filepath = os.path.join(BASE_DIR, "database/myDB.sqlite")

logDB_Filepath = os.path.join(BASE_DIR, "database/log.sqlite")

settingsDB_Filepath = os.path.join(BASE_DIR, "database/settings.sqlite")
...
def getSettingFromDB(setting):
    ...
    with conn:
        ...
        cur = conn.cursor()
        # print('debugging....Successfuly connected to the sqlite')
    ...
    conn.close()
    # print('debugging...endpoint settings', value)
    return value[0]
...
def setEndpointURL(oldUrl, newUrl):
    ...
    if checkIfEndpointIsReachable():
        print('debuggng---- checking if endpoint is reachable')
        return True

    # If the new URL is not reachable, set the old URL again and return false
    else:
        print('debugging--- the end point is not reachable')
...
def checkIfnamespaceManagerDatabase(fileWithPath):
...
#     # (e.g. http://www.semanticweb.org/ontologies/untitled-ontology-69#Artist)
...
def getURLOfPhrase(phrase):
    # Gets a phrase
    # e.g. http://www.semanticweb.org/ontologies/untitled-ontology-69#Artist or my_ont:Artist
    # and returns the url http://www.semanticweb.org/ontologies/untitled-ontology-69#
...
def getAllClassesFromGraph(my_graph):
    ...
    #sparql query to retrive s from the (s, rdf:type, o) where o is owl:Class or rdfs:Class object
    # query = "SELECT DISTINCT ?class WHERE {{?class rdf:type owl:Class} UNION {?class rdf:type rdfs:Class} .}"

    #updated query to retrieve core:prefLabel label_en
    query = "SELECT DISTINCT ?label_en WHERE {?class rdf:type owl:Class; core:prefLabel ?label_en .}"
    
    # query = "SELECT DISTINCT ?class WHERE {{?class rdf:type owl:Class} UNION {?class rdf:type rdfs:Class} " +\
    #         "UNION {?instance rdf:type owl:NamedIndividual . ?instance rdf:type ?class. ?class rdf:type owl:Class} .}"

    qres = executeSPARQLSelectToGraph(my_graph, query)
    # print('deugging...', [i for i in range(1000)])
    #print(qres.result)
    return [result[0] for result in qres.result]


def getAllSubclassesOfClass(my_graph, theClass):

    # sparql query to retrieve subclasses of a given class
    
    query = "SELECT DISTINCT ?subclass WHERE {?subclass rdfs:subClassOf <" + theClass + "> .}"
...
def getLabelsOfInstanceFromGraph(my_graph, instance):
    ...
    qres = executeSPARQLSelectToGraph(my_graph, str(query))
    labels = []
...
def openContextMenuInTree(treeWidget, caller = None, myGraph = None):
    ...
    # If a property is selected
        ...
        menu.addAction("Copy value to clipboard").triggered.connect(lambda: copyDataPropertyValueToClipboard(str(value)))
...
def searchTree(lineEdit, treeWidget):
        ...
        treeWidgetMyModel_iterator = QtWidgets.QTreeWidgetItemIterator(treeWidget)
        tree_item = treeWidgetMyModel_iterator.value()
        while (tree_item):
            tree_item.setForeground(0,QtGui.QBrush(QtGui.QColor("black")))
            tree_item.setForeground(0,QtGui.QBrush(QtGui.QColor("black")))
        ...
        if len(searchTerm) > 0:
            ...
            for item in items_containing_term:
                ...
                if searchTerm.lower() in item.text(0).lower():
                    item.setForeground(0,QtGui.QBrush(QtGui.QColor("blue")))

                if searchTerm.lower() in item.text(1).lower():
