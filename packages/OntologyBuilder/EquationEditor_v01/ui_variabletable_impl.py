...
from OntologyBuilder.EquationEditor_v01.ui_documentation_impl import UI_DocumentationDialog
from OntologyBuilder.EquationEditor_v01.ui_get_qudt_iri_impl import UI_QUDTFetch_IRI
from OntologyBuilder.EquationEditor_v01.ui_physunits_impl import UI_PhysUnitsDialog
from OntologyBuilder.EquationEditor_v01.ui_symbol_impl import UI_SymbolDialog
from OntologyBuilder.EquationEditor_v01.variable_framework import Units
from OntologyBuilder.EquationEditor_v01.variable_framework import simulateDeletion
...
class UI_VariableTableDialog(VariableTable):
    ...
    def __showDeleteDialog(self, selected_ID):
        ...
        if eqs == []:
            answer = makeMessageBox("no equations detelet variable?", buttons=["YES", "NO"], default="NO",
                                    infotext="delete ?")
            delete = answer == "YES"
    ...
    def __changeSymbol(self, variable):  # , forbidden_symbols):
        accessible_variables_per_class = self.variables.index_accessible_variables_on_networks[
            self.network]  # nameSpacesForVariableLabelGlobal
        forbidden_symbols = []
